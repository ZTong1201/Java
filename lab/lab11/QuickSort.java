...
public class QuickSort {
    ...
    private static <Item extends Comparable> void partition(
        ...
        while(!unsorted.isEmpty()) {
            Item checkValue = unsorted.dequeue();
            if(checkValue.compareTo(pivot) < 0) {
                less.enqueue(checkValue);
            } else if(checkValue.compareTo(pivot) > 0) {
                greater.enqueue(checkValue);
            } else {
                equal.enqueue(checkValue);
            }
        }
    }
