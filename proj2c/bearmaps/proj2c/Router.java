...
public class Router {
    ...
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g, List<Long> route) {
        ...
        int currDirection = 0;
        double prevBearing = NavigationDirection.bearing(g.lon(startVertex), g.lon(route.get(1)),
                g.lat(startVertex), g.lat(route.get(1)));
        double currDist = 0.0;
        ...
        for(int i = 1; i < route.size(); i++) {
            ...
            double currBearing = NavigationDirection.bearing(g.lon(prevVertex), g.lon(currVertex),
                    g.lat(prevVertex), g.lat(currVertex));
            int newDirection = NavigationDirection.getDirection(prevBearing, currBearing);


            if(prevVertex == startVertex) {
                if(getWayName(g, prevVertex, currVertex) != null) currWayName = getWayName(g, prevVertex, currVertex);
            } else {
                prevBearing = currBearing;
            }

            if(prevVertex == startVertex) {
                if(getWayName(g, prevVertex, currVertex) != null) {
                    currWayName = getWayName(g, prevVertex, currVertex);
                }
            } else {
                prevBearing = currBearing;
            }
            if(i != route.size() - 1 && wayNameSet(g, currVertex).contains(currWayName)) {
                currDist += g.estimatedDistanceToGoal(prevVertex, currVertex);
                continue;
            }
            if(i == route.size() - 1) {
                currDist += g.estimatedDistanceToGoal(prevVertex, currVertex);
            ...
            navigationDirection.distance = currDist;
            if(currWayName != null) navigationDirection.way = currWayName;
            navigationDirection.direction = currDirection;
    ...
    private static String getWayName(AugmentedStreetMapGraph g,long prev, long curr) {
        for(String prevWay : wayNameSet(g, prev)) {
            for(String currWay: wayNameSet(g, curr)) {
                if(prevWay.equals(currWay)) {
                    return prevWay;
                }
            }
        }
    ...
    private static Set<String> wayNameSet(AugmentedStreetMapGraph g, Long vertex) {
        List<WeightedEdge<Long>> neighbors = g.neighbors(vertex);
        Set<String> wayNames = new HashSet<>();
        for(WeightedEdge<Long> edge : neighbors) {
            if(!wayNames.contains(edge.getName()) && edge.from().equals(vertex)) {
                wayNames.add(edge.getName());
            }
        }
        return wayNames;
    }
