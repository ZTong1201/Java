...
public class PianoPlayer {
    ...
    public PianoPlayer(InputStream source) {
        try {
    ...
    public PianoPlayer(File source) {
        try {
            sequence = MidiSystem.getSequence(source);
        } catch (IOException | InvalidMidiDataException e) {
            e.printStackTrace();
    ...
    private void initialize() {
        strings = new GuitarString[88];
        vol = new double[88];
        for (int i = 0; i < strings.length; i++) {
            strings[i] = new GuitarString(440.0 * Math.pow(2.0, (i - 49.0) / 12.0));
            vol[i] = 0.0;
        }
    }
